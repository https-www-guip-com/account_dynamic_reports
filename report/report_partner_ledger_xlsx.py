...
class InsPartnerLedgerXlsx(models.AbstractModel):
    ...
    def prepare_report_filters(self, filter):
        ...
        if filter:
            ...
            # Journals
            self.row_pos_2 += 2
            self.sheet_2.write_string(self.row_pos_2, 0, _('Diarios'),
                                    self.format_header)
            ...
            # Partners
            self.row_pos_2 += 1
            self.sheet_2.write_string(self.row_pos_2, 0, _('Socios'),
                                                 self.format_header)
            ...
            # Partner Tags
            self.row_pos_2 += 1
            self.sheet_2.write_string(self.row_pos_2, 0, _('Etiquetas De Socios'),
                                      self.format_header)
            ...
            # Accounts
            self.row_pos_2 += 1
            self.sheet_2.write_string(self.row_pos_2, 0, _('Cuentas'),
                                    self.format_header)
    ...
    def prepare_report_contents(self, data, acc_lines, filter):
        ...
        if filter.get('include_details', False):
            self.sheet.write_string(self.row_pos, 0, _('Fecha'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 1, _('JRNL'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 2, _('Socios'),
                                    self.format_header)
            # self.sheet.write_string(self.row_pos, 3, _('Ref'),
            #                         self.format_header)
            self.sheet.write_string(self.row_pos, 3, _('Movimiento'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 4, _('Etiqueta de entrada'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 5, _('Debito'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 6, _('Credito'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 7, _('Balance'),
                                    self.format_header)
        else:
            self.sheet.merge_range(self.row_pos, 0, self.row_pos, 4, _('Socios'), self.format_header)
            self.sheet.write_string(self.row_pos, 5, _('Debito'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 6, _('Credito'),
                                    self.format_header)
                    ...
                    for sub_line in sub_lines:
                        if sub_line.get('move_name') == 'Initial Balance':
                            ...
                            self.sheet.write_number(self.row_pos, 7, float(acc_lines[line].get('balance')),
                                                    self.line_header_light_initial)
                        elif sub_line.get('move_name') not in ['Balance Inicial','Ending Balance']:
                            self.row_pos += 1
