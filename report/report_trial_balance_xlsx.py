...
class InsTrialBalanceXlsx(models.AbstractModel):
    ...
    def prepare_report_filters(self, data, filter):
        ...
        if filter:
            ...
            # Accounts
            self.row_pos_2 += 1
            self.sheet_2.write_string(self.row_pos_2, 0, _('Cuentas Analiticas'),
                                      self.format_header)
    ...
    def prepare_report_contents(self, acc_lines, retained, subtotal, filter):
        ...
        self.row_pos += 3
        self.sheet.merge_range(self.row_pos, 1, self.row_pos, 3, 'Balance Inicial', self.format_merged_header)
        ...
        self.sheet.merge_range(self.row_pos, 7, self.row_pos, 9, 'Balance final', self.format_merged_header)

        self.row_pos += 1

        self.sheet.write_string(self.row_pos, 0, _('Cuentas'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 1, _('Debito'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 2, _('Credito'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 3, _('Balance'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 4, _('Debito'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 5, _('Credito'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 6, _('Balance'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 7, _('Debito'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 8, _('Credito'),
                                self.format_header)
