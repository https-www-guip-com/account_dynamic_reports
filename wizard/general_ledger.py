...
class InsGeneralLedger(models.TransientModel):
    ...
    target_moves = fields.Selection(
        [('all_entries', 'Todas las entradas'),
         ('posted_only', 'Solo publicado')], string='Movimientos de destino',
        default='posted_only', required=True
    )
    sort_accounts_by = fields.Selection(
        [('date', 'Fecha'), ('journal', 'Diario y Socios')], string='Ordenar por',
    ...
    account_tag_ids = fields.Many2many(
        'account.account.tag', string='Etiquetas de cuentas'
    )
    analytic_ids = fields.Many2many(
       'account.analytic.account', string='Cuentas Analiticas'
    )
    analytic_tag_ids = fields.Many2many(
        'account.analytic.tag', string='Etiquetas Analiticas'
    )
    ...
    def write(self, vals):
        ...
        if vals.get('account_tag_ids'):
            vals.update({'account_tag_ids': [(5, 0, 0)] + [(4, j) for j in vals.get('account_tag_ids') if type(j) is not list] + vals.get('account_tag_ids')})
        if vals.get('account_tag_ids') == []:
