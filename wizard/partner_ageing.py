...
class InsPartnerAgeing(models.TransientModel):
    ...
    as_on_date = fields.Date(string='Como en la fecha', required=True, default=fields.Date.today())
    ...
    include_details = fields.Boolean(string='Incluir Detalles', default=True)
    type = fields.Selection([('receivable','Solo cuentas por cobrar'),
                              ('payable','Solo cuentas a pagar')], string='Tipo')
    partner_type = fields.Selection([('customer', 'Solo cliente'),
                             ('supplier', 'Solo proveedor')], string='Tipo Socios')
