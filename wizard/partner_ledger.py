...
class InsPartnerLedger(models.TransientModel):
    ...
    financial_year = fields.Selection(
        ...
        string='Año financiero', default=_get_default_financial_year)
    ...
    date_range = fields.Selection(
        ...
        string='Rango de fechas', default=_get_default_date_range
    )
    target_moves = fields.Selection(
        [('all_entries', 'Todas las entradas'),
         ('posted_only', 'Solo publicado')], string='Movimientos',
        default='posted_only', required=True
    )
    display_accounts = fields.Selection(
        [('all', 'Todas'),
         ('balance_not_zero', 'Con saldo no igual a cero')], string='Mostrar cuentas',
        default='balance_not_zero', required=True
    )
    balance_less_than_zero = fields.Boolean(string='Con saldo menor que cero')
    balance_greater_than_zero = fields.Boolean(string='Con saldo mayor que cero')
    type = fields.Selection(
        [('receivable', 'Solo cuentas por cobrar'),
         ('payable', 'Solo pagadero')],
        string='Tipo de Cuentas',required=False
    )
    initial_balance = fields.Boolean(
        string='Incluir saldo inicial', default=True
    )
    reconciled = fields.Selection([('reconciled','Solo reconciliado'),
                                   ('unreconciled','Solo no reconciliados')],
                                  string='Tipo de reconciiacion')
    ...
    partner_ids = fields.Many2many(
        'res.partner', string='Socios'
    )
    company_id = fields.Many2one(
        'res.company', string='Compañias',
        default=_get_default_company
    )
    include_details = fields.Boolean(
        string='Incluir Detalles', default=True
    )
    partner_category_ids = fields.Many2many(
       'res.partner.category', string='Etiquetas de socios',
    )
    ...
    def write(self, vals):
        ...
        if vals.get('partner_category_ids'):
            vals.update({'partner_category_ids': [(5, 0, 0)] + [(4, j) for j in vals.get('partner_category_ids') if type(j) is not list] + vals.get('partner_category_ids')})
        if vals.get('partner_category_ids') == []:
